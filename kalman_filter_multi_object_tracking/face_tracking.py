...
def main():
    ...
    #cap = cv2.VideoCapture('/home/batrumot/study/MMDB/test2.mp4')
    ...
    while(True):
        ...
        centers = detector.detect_face(frame)
                ...
                if (len(tracker.tracks[i].trace) > 1):
                    for j in range(len(tracker.tracks[i].trace)-1):
                        ...
                        cv2.line(frame, (int(x1), int(y1)), (int(x2), int(y2)),
                                 track_colors[clr], 2)
                        img2 = cv2.rectangle(frame, (int(x2 - 10),int(y2 - 10)), (int(x2+10),int(y2+10)), (0,0,255),1)
